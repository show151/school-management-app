...
- **新規登録時のメールアドレス確認 (Email Verification)**:
  - 登録時に有効期限付きのワンタイムトークン（`verificationToken`）を発行してメール送信。
  - 認証リンクをクリックすることで初めてアカウントが有効化されるセキュアなフロー。
- **パスワードリセット機能**:
  - 安全なワンタイムトークン（`resetToken`）を用いたパスワード再設定フロー。
  - トークンは DB にハッシュ化して保存し、万が一の漏洩時も平文を残さない設計。
- **bcrypt ハッシュ化**: salt round = 10
  - ユーザーのパスワードは強力にハッシュ化され保存されます。
- **パスワード強度要件**:
  - 8〜64文字
...
`middleware.ts` を用いた強固なアクセス制御を実装しています:

- **ロールベースのアクセス制御 (RBAC)**: 一般ユーザーと管理者を明確にロール分離し、`/admin` や `/api/admin` は管理者権限（admin トークン）を持つユーザーのみにアクセスを許可。
- **IPアドレス制限 (管理者向け)**: 環境変数 `ADMIN_IP_WHITELIST` で許可されたIPアドレスからのみ管理者機能を利用可能にし、不正なアクセスを遮断。
- **保護されたルートへのアクセス制御**: 未認証ユーザーをログイン画面へリダイレクト。
- **JWT トークン検証**: Edge Runtime で高速かつ安全にトークンを検証。
- **期限切れトークンの自動削除**: トークンの期限切れや不正を検知した場合、安全策として即座にCookieから削除してログアウトさせる。

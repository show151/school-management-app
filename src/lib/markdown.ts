...
export function markdownToHtml(md: string): string {
  ...
  function addPlaceholder(html: string) {
    const id = `@@MDPLACEHOLDER${placeholders.length}@@`;
  ...
  s = s.replace(/`([^`]+)`/g, (_m, code) => {
    return addPlaceholder(`<code class="bg-[var(--card)] px-1.5 py-0.5 rounded border border-[var(--border)] text-sm font-mono break-all">${code}</code>`);
  });
  ...
  s = s.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_m, text, url) => {
    ...
    return addPlaceholder(`<a href="${safeUrl}" target="_blank" rel="noopener noreferrer" class="text-[var(--primary)] hover:underline break-all">${formattedText}</a>`);
  });
  ...
  // 9. Paragraphs: wrap lines separated by blank lines

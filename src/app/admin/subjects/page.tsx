...
export default function AdminSubjectsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!res.ok) { alert("教科の登録に失敗しました。"); return; }
    setName("");
    reload();
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!res.ok) { alert("教科の削除に失敗しました。"); return; }
    reload();
  };
    ...
    <div className="min-h-screen bg-[var(--background)] admin-theme">
      <div className="container-responsive py-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-2xl font-bold text-[var(--foreground)]">教科登録</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card">
          ...
          {nameError && <small className="text-xs text-red-600 mt-1 block">{nameError}</small>}
        </form>

        <div className="card">
          <h2 className="mb-3 text-base font-bold text-[var(--foreground)]">登録済み教科</h2>
          {subjects.length === 0 ? (
            <p className="text-sm text-[var(--muted)]">まだ教科が登録されていません。</p>
          ) : (
